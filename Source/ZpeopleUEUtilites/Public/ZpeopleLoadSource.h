...
public:
  ...
  //从本地加载贴图 保存为bin文件 第二次加载bin文件 
	UFUNCTION(BlueprintCallable, Category = "Zpeople|Utilityes")
		static	UTexture2D * LoadTexAndSaveBinFromFile(const FString & FilePath, const bool & isSave ,bool & IsValid, int32 & Width, int32 & Height);
	...
	//从本地加载贴图 保存缩略图
	UFUNCTION(BlueprintCallable, Category = "Zpeople|Utilityes")
	static	UTexture2D * LoadTexAndSaveThumbnailFromFile(const FString & FilePath, const bool & isRid, bool & IsValid, int32 & Width, int32 & Height);

	static bool ImageShrink(const TArray<FColor> &Bitmap, const int32 Width, const int32 Height, const float Ratio,
		TArray<FColor> &NewBitmap, int32 &NewWidth, int32 &NewHeight,bool isRid);
	 
	static  bool IsNeedRid(const int32 w, const int32 h, const int32 Width, const int32 Height);
